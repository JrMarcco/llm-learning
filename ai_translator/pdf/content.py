...
class Content:
    ...
    def set_translation(self, translation, status):
        if not self.check_translation_type(translation):
            raise ValueError(f"Invalid translation type. Expected {self.content_type}, got {type(translation)}")
        self.translation = translation
        self.status = status
...
class TableContent(Content):
    def __init__(self, data, translation=None):
        ...
        # Verify if the number of rows and columns
    ...
    def set_translation(self, translation, status):
        try:
            if not isinstance(translation, str):
                raise ValueError(f"Invalid translation type. Expected str, got {type(translation)}")
            ...
            LOG.error(f"error occurred during table translation: {e}")
    ...
    def update_item(self, row_idx, col_idx, new_val, translated=False):
        target_df = self.translation if translated else self.original
        target_df.at[row_idx, col_idx] = new_val
